...
const Category = () => {
    ...
    useEffect(() => {
        const fo = () => {
            axios.get(`${Config.URL}/categorys`)
                .then((res) => {
                    setData(res.data)
                })
                .catch((err) => {
    ...
    const subBaza = (id) => {
        axios.get(`${Config.URL}/categorys/sub/${id}`)
            .then((res) => {
                setSubData(res.data)
            })
            .catch((err) => {
    ...
    const sendData = (e) => {
        e.preventDefault()
        axios.post(`${Config.URL}/categorys`, addData)
            .then((res) => {
    ...
    const DeleteCategory = (id) => {
        if (window.confirm("Delete the item?")) {
            axios.delete(`${Config.URL}/categorys/${id}`, addData)
                .then((res) => {
                    window.location.reload();
                })
                .catch((err) => {
    ...
    return (
        <div className="wrap mt-5">
            <div className="nav d-flex justify-content-around">
                <div className="menu_item btn btn-primary"
            ...
            <div className="category_content text-center mt-5">
                <div className="ico" onClick={() => changeModal('d-inline cat_con d-flex justify-content-center')}>
                    ...
                    <div className="left">
                        <h1>Bo'limlar</h1>
                        {
                            data.map((item, index) => {
                                return (
                                    <div key={index}>
                                        <h3 className="btn btn-success d-inline-block" onClick={() => subBaza(item._id)}>
                                            {index + 1}. {item.title}

                                        </h3>
                                        <span className="del" onClick={() => DeleteCategory(item._id)}>
                    ...
                    <div className="cat_conten">
                        ...
                        <form className="text-center" onSubmit={sendData}>
                            ...
                            <select
                                ...
                                onChange={changeHandler}                        >
                                {[{ title: "Bo'lim tanlang", _id: "null" }].concat(data).map((item, index) => {
                                    return (
                            ...
                            <button
                                className="btn btn-primary m-3">AddCategory
                            </button>
                        </form>
                    </div>
                </div>
